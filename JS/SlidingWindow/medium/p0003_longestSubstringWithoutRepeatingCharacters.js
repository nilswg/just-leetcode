...
// 解題重點
// 1. 單純以滑動窗(雙指標)去實作暴力解，O(N^2) / O(1)
// 2. 再同樣以滑動窗，配合HashMap優化算法，O(N) / O(N)
 ...
 *
 * 時間複雜度分析 : 
 * 當內層迴圈檢查滑動窗中是否有重覆元素時，由於每次都會檢查到滑動窗中的最後一個元素，
 * 又以最糟情況來看，也就是一個從頭到尾皆完全不重覆的字串，隨著外層迴圈其滑動窗不斷變大，
 * 總共檢查時間為 1 + 2 + ... + n-1，即 (1+(n-1))*(n-1)/2 = n*(n-1)/2，
 * 其時間複雜度為 O(N^2);
 * 
 ...
 * 
 * 對比前一個僅使用滑動窗求解，我們透過雜湊表去記憶出現過的元素，縮短其每次查找滑動窗內元素的時間。
 *
 * 1. 透過雙指針，lt, i 去控制窗口的大小，
